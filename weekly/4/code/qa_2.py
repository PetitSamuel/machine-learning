...
for degree in [1, 10, 40]:
    poly = PolynomialFeatures(degree)
    X_poly = poly.fit_transform(X)
    model = LogisticRegression(solver='liblinear', penalty='l1').fit(X_poly, y)
...
for C in C_values:
    ...
    model = LogisticRegression(
        solver='liblinear', penalty='l1', C=C).fit(X_poly, y)
    scores.append(model.score(X_poly, y))
...
ax.errorbar(C_values, scores, label='Mean accuracy',
            yerr=C_values, uplims=True, lolims=True)
ax.set_ylabel("Score (mean accuracy) of the model")
ax.set_xlabel("C")
ax.set_title(
    "Cross-validation of the L1 penalty term C for a Logistic Regression model")
ax.legend(loc='lower right')
